...
within the namespace is preserved. A range query `foo <= x < fop` returns all
keys within `foo:` in order.
...
encoding needs to be order preserving, i.e. for every `a < b` the
`encoding(a) < encoding(b)`. One encoding that fulfills this requirement is hex,
or more precisely the ascii encoding of hex `ascihex(x) := ascii(hex(x))`. We
use lower case hex for no reason other than we need to fix casing. The output
range of asciihex is `'0' - '9'` (i.e. 48-57) and `'a' - 'f'` (i.e. 97-102).
When we asciihex encode the namespace and the key, we can use the null byte 0x00
as a separator: `asciihex(namespace) | 0x00 | asciihex(key)`.
...
This approach can easily be generalized to multi-level namespaces. It gives is
